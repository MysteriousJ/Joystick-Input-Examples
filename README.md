...
### XInput
The only controllers you're likely to find that don't use HID are Microsoft's own XBox controllers. These use custom communication protocols and Microsoft introduced a new API to work with them. XBox 360 controllers use [XUSB protocol](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-xusbi/c79474e7-3968-43d1-8d2f-175d47bef43e) and XBox One controllers use [Game Input Protocol](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-gipusb/e7c90904-5e21-426e-b9ad-d82adeee0dbc) (GIP). There's a driver for each: xboxgip.sys and xusb22.sys. We'll refer to the driver and the API as XInput. It supports up to 4 controllers at a time, with a light on the controller indicating the player index.
...
XBox controllers also work on older games that only implemented support for HID. The XInput driver creates a virtual HID device and forwards inputs to it. Effectively there are two copies of each XBox controller on your system, the XInput version and the HID version. The HID version comes with a downside: the XInput driver combines both shoulder tiggers into one axis when converted to HID. Pressing RT makes the axis go in the negative direction, and LT makes it go in the positive direction. If you press both at the same time, it's the same as pressing neither. [Windows Registry values](https://github.com/nefarius/ViGEmBus/issues/40#issuecomment-619889180) can change this behavior to separate the triggers, though a game would require administrative privileges to automatically set them.
...
### XBox controllers
If you really don't like the XInput API, it's possible to work with XBox controllers without it. [Mārtiņš Možeiko has an excellent example program](https://gist.github.com/mmozeiko/b8ccc54037a5eaf35432396feabbe435) that uses file I/O to interface with the loaded XInput driver, and [Dave Madison has a great walkthrough of using Wireshark](https://www.partsnotincluded.com/understanding-the-xbox-360-wired-controllers-usb-data/) to reverse engineer the data needed. If you want to skip the driver, TheNathannator wrote a walkthrough that breaks down [how to use the GIP interface](https://gist.github.com/TheNathannator/bcebc77e653f71e77634144940871596).
